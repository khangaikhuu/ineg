...
public class MWorkCondition implements Serializable {
    ...
    @Column(name = "name")
    private String name;
    @OneToMany(mappedBy = "workConditionId")
    private List<MStaff> mStaffList;

    public MWorkCondition() {
    }

    public MWorkCondition(Integer id) {
        this.id = id;
    }
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof MWorkCondition)) {
            return false;
        }
        MWorkCondition other = (MWorkCondition) object;
